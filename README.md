...
**OpenClaw** is a personal AI assistant that runs on your own computer. Think of it as a local agent that can run tasks on a schedule, connect to your messaging apps, and use tools you give it. In this project OpenClaw is the engine — it wakes up every morning, kicks off the scan, and sends you the result.

**The OCC scraper** is a Python script that opens a browser, searches the OCC website for memos related to your tickers, and downloads the PDF documents. It then extracts the text from those PDFs so it can be read and interpreted.

**Claude** reads the raw PDF text and turns it into the plain-English alert you receive. It knows the mechanics of how each corporate action type affects your options — what changes, what stays the same, what the exercise math looks like — and writes it out in a format that is actually useful to a trader.

The three pieces connect like this:
```
OpenClaw wakes up at 7am
        ↓
Runs the scraper → finds new OCC memos → reads the PDFs
        ↓
Sends the text to Claude → Claude writes the plain-English alert
        ↓
OpenClaw delivers the alert to your Telegram
```

You do not need to touch any of it after setup. It just runs.
...
Just talk to Claude via Telegram:
...
| File | What it does |
|------|-------------|
| `SKILL.md` | Instructions that tell OpenClaw how to run scans and format alerts |
| `scripts/scrape_occ.py` | The scraper that fetches and reads OCC PDFs |
